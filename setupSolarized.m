...
sol.base01  = [ 88 110 117] / 255; % base01    #586e75 10/7 brgreen  240 #585858 45 -07 -07  88 110 117 194  25  46
sol.base00  = [101 123 131] / 255; % base00    #657b83 11/7 bryellow 241 #626262 50 -07 -07 101 123 131 195  23  51
sol.base0   = [131 148 150] / 255; % base0     #839496 12/6 brblue   244 #808080 60 -06 -03 131 148 150 186  13  59
sol.base1   = [147 161 161] / 255; % base1     #93a1a1 14/4 brcyan   245 #8a8a8a 65 -05 -02 147 161 161 180   9  63
sol.base2   = [238 232 213] / 255; % base2     #eee8d5  7/7 white    254 #e4e4e4 92 -00  10 238 232 213  44  11  93
sol.base3   = [253 246 227] / 255; % base3     #fdf6e3 15/7 brwhite  230 #ffffd7 97  00  10 253 246 227  44  10  99
sol.yellow  = [181 137   0] / 255; % yellow    #b58900  3/3 yellow   136 #af8700 60  10  65 181 137   0  45 100  71
sol.orange  = [203  75  22] / 255; % orange    #cb4b16  9/3 brred    166 #d75f00 50  50  55 203  75  22  18  89  80
sol.red     = [220  50  47] / 255; % red       #dc322f  1/1 red      160 #d70000 50  65  45 220  50  47   1  79  86
sol.magenta = [211  54 130] / 255; % magenta   #d33682  5/5 magenta  125 #af005f 50  65 -05 211  54 130 331  74  83
sol.violet  = [108 113 196] / 255; % violet    #6c71c4 13/5 brmagenta 61 #5f5faf 50  15 -45 108 113 196 237  45  77
sol.blue    = [ 38 139 210] / 255; % blue      #268bd2  4/4 blue      33 #0087ff 55 -10 -45  38 139 210 205  82  82
sol.cyan    = [ 42 161 152] / 255; % cyan      #2aa198  6/6 cyan      37 #00afaf 60 -35 -05  42 161 152 175  74  63
sol.green   = [133 153   0] / 255; % green     #859900  2/2 green     64 #5f8700 60 -20  65 133 153   0  68 100  60
mat.black   = [  0   0   0] / 255;
mat.white   = [255 255 255] / 255;
mat.blue    = [  0   0 255] / 255;
mat.green   = [ 34 139  34] / 255;
mat.violet  = [160  32 240] / 255;
mat.red178  = [178   0   0] / 255;
mat.red230  = [230   0   0] / 255;
mat.golden  = [178 140   0] / 255;
mat.orange  = [255 148   0] / 255;
mat.chamois = [238 225 180] / 255;
mat.iceblue = [202 232 232] / 255;
mat.persian = [  0 163 163] / 255;
...
if strcmp(slrzd,'light')
    ...
    kwd  = sol.green;   % Set 'Keywords' color
    cmt  = sol.base1;   % Set 'Comments' color
    str  = sol.cyan;    % Set 'Strings' color
    ustr = sol.violet;  % Set 'Unterminated strings' color
...
elseif strcmp(slrzd,'dark')
    ...
    kwd  = sol.green;   % Set 'Keywords' color
    cmt  = sol.base01;  % Set 'Comments' color
    str  = sol.cyan;    % Set 'Strings' color
    ustr = sol.violet;  % Set 'Unterminated strings' color
    scmd = sol.yellow;  % Set 'System commands' color
    errs = sol.red;     % Set 'Errors' color
...
elseif strcmp(slrzd,'default')
    ...
    cmt  = mat.green;   % Set 'Comments' color
